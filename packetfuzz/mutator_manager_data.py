...
class FieldMetadata:
    ...
    field_format: Optional[str] = None  # Special format constraints (e.g., 'tcp_options')
...
class MutatorManagerData:
    ...
    def __init__(self, fuzz_config: FuzzConfig):
        ...
        # Set basic counts immediately for property access
        if isinstance(self.original_packets, list):
            self.total_packets = len(self.original_packets)
        else:
        ...
        # Initialize field statistics (will be updated during preprocessing)
        self.total_fields = 0
    ...
    def is_single_packet(self) -> bool:
        """Check if this data tracker contains only a single packet."""
        return self.total_packets == 1

    @property
    def packet_list(self) -> List[Packet]:
        """Get the list of original packets."""
        if isinstance(self.original_packets, list):
            return self.original_packets
        else:
            return [self.original_packets]
...
# Complex preprocessing, configuration resolution, and field analysis logic
    ...
    def _categorize_field_type(self, layer: Packet, field_name: str) -> str:
        ...
        """
        from packetfuzz.utils.packetfuzz_utils import get_field_type_chain
        from packetfuzz.utils.scapy_utils import get_layer_name
    ...
    def _extract_field_constraints(self, layer: Packet, field_desc: Field, layer_name: str, field_name: str) -> Dict[str, Any]:
        ...
        from scapy.fields import EnumField, IntEnumField, StrField, ByteField, XByteField, ShortField, XShortField, IntField, XIntField
        
        # Numeric field constraints based on field type
        if isinstance(field_desc, (ByteField, XByteField)):
            constraints['min_value'] = 0
            constraints['max_value'] = 255
        elif isinstance(field_desc, (ShortField, XShortField)):
            constraints['min_value'] = 0
            constraints['max_value'] = 65535
        elif isinstance(field_desc, (IntField, XIntField)):
            constraints['min_value'] = 0
            constraints['max_value'] = 4294967295
        ...
        # Special handling for TCP options field
        if layer_name == 'TCP' and field_name == 'options':
            # TCP options should be lists of tuples or empty
            constraints['field_format'] = 'tcp_options'
    ...
    def _get_default_config(self, field_metadata: FieldMetadata, 
        ...
        """Get default dictionaries for a field. Default values are not loaded from dictionaries."""
    ...
    def _calculate_layer_depth(self, layer):
        ...
        """
        from packetfuzz.utils.scapy_utils import calculate_layer_depth_below
    ...
    def _safe_ip_assign(self, layer_obj, field_name: str, value: Any) -> Tuple[bool, str]:
        """
        Safely assign a value to an IP field without DNS lookup delays.
        
        This method prevents DNS resolution timeouts that occur when Scapy
        attempts to resolve invalid string values as hostnames. The method
        tries multiple strategies in order of preference:
        
        1. Integer values (fastest, no DNS)
        2. Valid IP address strings (validated with socket.inet_aton)  
        3. String to integer conversion
        4. Direct assignment bypass for invalid strings
        
        Args:
            layer_obj: Scapy layer object (e.g., IP layer)
            field_name: Name of the field to assign (e.g., 'src', 'dst')
        ...
        Reference:
            See docs/SCAPY_DNS_BEHAVIOR.md for detailed analysis
        """
        try:
            # Method 1: Integer values (fastest, no DNS lookup)
            if isinstance(value, int):
                setattr(layer_obj, field_name, value)
                return True, f'integer: {value}'
                
            # Method 2: Valid IP address strings
            ...
            if isinstance(value, str):
                try:
                    int_val = int(value)
                    setattr(layer_obj, field_name, int_val)
                    return True, f'string->int: {value} -> {int_val}'
                except ValueError:
                    pass
                    
            # Method 4: Direct assignment to bypass DNS resolution
            # This prevents 4+ second DNS timeouts for invalid hostnames
            object.__setattr__(layer_obj, field_name, value)
            return True, f'bypass DNS: {value}'
    ...
    def validate_and_assign(self, field, packet_index: int = 0, value: Any = None) -> bool:
        ...
        try:
            ...
            # Special handling for TCP options field
            if field_metadata.field_format == 'tcp_options':
                ...
                if kind in ('numeric', 'flags', 'enum'):
                    ...
                    if kind in ('numeric', 'flags'):
                        ...
                        if min_value is not None and max_value is not None:
                            logger.debug(f"validate_and_assign: Clamped {assign_value} to {normalized} (range: {min_value}-{max_value})")
                    elif kind == 'enum':
            ...
            try:
                ...
                if (field_metadata.layer_name == 'IP' and fname in ['src', 'dst']):
                    success, description = self._safe_ip_assign(layer_clone, fname, normalized)
                    if not success:
                        logger.debug(f"validate_and_assign: Safe IP assignment failed for {fname}: {description}")
                        return False
                    logger.debug(f"validate_and_assign: Safe IP assignment for {fname}: {description}")
                else:
                    # Use standard assignment for non-IP fields
            ...
            try:
                # Use safe IP assignment for IP fields to prevent DNS resolution delays
                if (field_metadata.layer_name == 'IP' and fname in ['src', 'dst']):
                    success, description = self._safe_ip_assign(layer, fname, normalized)
                    if not success:
                        logger.debug(f"validate_and_assign: Final safe IP assignment failed for {fname}: {description}")
                        return False
                    logger.debug(f"validate_and_assign: Final safe IP assignment for {fname}: {description}")
                else:
                    # Use standard assignment for non-IP fields
    ...
    def record_field_mutation(self, field_key: str, packet_index: int, 
        ...
        if field_metadata:
            ...
            # Debug logging for when field is not found
            logger.debug(f"record_field_mutation: Could not find field {field_key} at packet index {packet_index} (success={success}, mutator={mutator_name})")
